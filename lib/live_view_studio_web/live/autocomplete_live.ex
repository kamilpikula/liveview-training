defmodule LiveViewStudioWeb.AutocompleteLive do
  ...
  def handle_event("suggestion-city", %{"city" => prefix}, socket) do
    socket = assign(socket,
  ...
  def handle_info({:run_zip_search, zip}, socket) do
    case Stores.search_by_zip(zip) do
     [] ->
        socket =
          socket
          |> put_flash(:info, "No stores matching for #{zip}")
  ...
  def handle_info({:run_city_search, city}, socket) do
    ...
    [] ->
        socket =
          socket
          |> put_flash(:info, "No city matching for #{city}")
          |> assign(stores: [], loading: false)
  ...
  def render(assigns) do
    ...
    <div id="search">
      ...
      <form phx-submit="city-search" phx-change="suggestion-city">
